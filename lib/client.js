    ...
    async initialize() {
        try {
            ...
            this.socket = makeWASocket({
                ...
                browser: ['MATDEV Bot', 'Desktop', '1.0.0'], // Changed to Desktop for better compatibility
                ...
                connectTimeoutMs: 60000,    // Longer timeout for QR scanning
                ...
                // Reduce MAC errors by being more lenient with message decryption
                maxMsgRetryCount: 5,
        ...
        this.socket.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'close') {
                ...
                if (statusCode === DisconnectReason.connectionReplaced || 
                    ...
                    this.logger.info('🔄 Connection needs fresh authentication. Restarting for QR code...');
                ...
                // Handle specific error patterns from latest Baileys issues
                if (errorMessage.includes('Stream Errored (conflict)') || 
                    errorMessage.includes('Stream Errored (restart required)')) {
                    this.logger.info('🔄 Stream conflict detected. Restarting connection...');
                    // Don't increment reconnect attempts for this - it's a known QR scan issue
                    setTimeout(() => {
                        this.initialize();
                    }, 3000); // Short delay before restart
                    return;
                }
                ...
                if ((errorMessage.includes('Connection Failure') && this.reconnectAttempts >= 3) ||
                    (errorMessage.includes('badSession') && this.reconnectAttempts >= 2)) {
        ...
        this.socket.ev.on('messages.upsert', async (m) => {
            try {
                ...
                if (error.message && error.message.includes('Bad MAC')) {
                    this.logger.warn('🔐 Message decryption failed (Bad MAC) - this is normal during session sync');
                } else {
