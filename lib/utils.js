...
class Utils {
    ...
    /**
     * Normalize JID to standard WhatsApp format for permissions
     */
    static normalizeJid(jid) {
        if (!jid) return '';

        // Handle @lid format - these are LinkedDevice IDs that need special handling
        if (jid.includes('@lid')) {
            // For @lid format, we need to extract the actual phone number
            // The format is usually like "185534701924401@lid" where the number contains the actual phone
            const lidNumber = jid.split('@')[0];

            // LID numbers are usually longer and contain the actual phone number
            // Try to extract a valid phone number from it
            if (lidNumber.length > 10) {
                // Extract last 10-13 digits as potential phone number
                const phoneMatch = lidNumber.match(/(\d{10,13})$/);
                if (phoneMatch) {
                    return `${phoneMatch[1]}@s.whatsapp.net`;
                }
            }

            // Fallback: use the full LID number
            return `${lidNumber}@s.whatsapp.net`;
        }

        // Extract phone number part - handle format like "2347018091555:72@s.whatsapp.net"
        const phoneNumber = jid.split('@')[0].split(':')[0];

        // Return standardized format for individual users
        if (jid.includes('@s.whatsapp.net')) {
            return `${phoneNumber}@s.whatsapp.net`;
        }

        // Keep group format as is but clean it
        if (jid.includes('@g.us')) {
            const groupId = jid.split('@')[0].split(':')[0];
            return `${groupId}@g.us`;
        }

        // Default to individual user format
        return `${phoneNumber}@s.whatsapp.net`;
    }
