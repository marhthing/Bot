...
class PermissionManager {
    ...
    async allowCommand(jid, command) {
        const normalizedJid = require('./utils').Utils.normalizeJid(jid);
        const userPermissions = this.permissions.get(normalizedJid) || [];
        if (!userPermissions.includes(command)) {
            userPermissions.push(command);
            this.permissions.set(normalizedJid, userPermissions);
            await this.savePermissions();
            this.logger.info(`✅ Allowed command '${command}' for ${normalizedJid}`);
            return true;
        }
    ...
    async removeCommand(jid, command) {
        ...
        if (index > -1) {
            ...
            if (userPermissions.length === 0) {
                this.permissions.delete(normalizedJid);
            } else {
                this.permissions.set(normalizedJid, userPermissions);
            }
            await this.savePermissions();
            this.logger.info(`❌ Removed command '${command}' for ${normalizedJid}`);
            return true;
        }
    ...
    hasPermission(jid, command) {
        ...
        if (userPermissions.includes(command) || userPermissions.includes('*')) {
            this.logger.debug(`✅ Permission granted for ${normalizedJid} to use '${command}'`);
            return true;
        }
        
        this.logger.debug(`❌ Permission denied for ${normalizedJid} to use '${command}'`);
        return false;
    }
